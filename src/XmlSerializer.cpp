	...
	for (unsigned i = 0; i < n; ++i) {
		...
		if (value) {
			if (as_link) {
                ...
                if (instance->type() == Type::IfcPropertySingleValue && argument_name == "NominalValue") {
                    child.put(stream.str(), instance->getArgument(2)->toString());
                } else {
                    child.put(stream.str(), *value);
                }
			}
		} else if (instance->type() == Type::IfcPropertySingleValue) {
            std::stringstream stream;
            stream << "<xmlattr>." << argument_name;
            child.put(stream.str(), "");
        }
	}
...
void XmlSerializer::finalize() {
    ...
    // Add svg data. Do not output unused items
	root.add_child("ifc",          svg_data);
