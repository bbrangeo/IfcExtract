...
void SvgSerializer::write(const IfcGeom::BRepElement<real_t>* o)
{
    ...
    double volume = 0.;
    double shape_zmin = inf;
    double shape_zmax = -inf;
	...
	for (IfcGeom::IfcRepresentationShapeItems::const_iterator it = o->geometry().begin(); it != o->geometry().end(); ++ it) {
		...
		// 2) Half way from geometry bounding box if the object has a height less than 2m
        ...
        volume += abs(IfcGeom::Kernel::shape_volume(moved_shape));
        if (zmin < shape_zmin ) {
            shape_zmin = zmin;
        }
        if (zmax > shape_zmax ) {
    ...
    if (volume > 0) {
        std::string guid = object_id(o);
        zmin_map.insert({guid, shape_zmin});
        zmax_map.insert({guid, shape_zmax});
        volume_map.insert({guid, volume});
    }
}
...
void SvgSerializer::finalize() {
	...
	for (it = paths.begin(); it != paths.end(); ++it) {
        ...
        std::string guid = it->second.first.substr(12, 22);
        if (zmin_map.count(guid) > 0) {
            double zmin = zmin_map[guid];
            double zmax = zmax_map[guid];
            double volume = volume_map[guid];
            svg_file << "        <g " << it->second.first << " zmin=\"" << zmin 
                << "\" zmax=\"" << zmax << "\" volume=\"" << volume << "\">\n";
        } else {
